...
dissect_cat(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
	...
	while (pos < tvb_length(tvb)) {
		...
		case 0x0e:	/* tone */
			if (len < 1)
				break;
			proto_tree_add_item(elem_tree, hf_ctlv_tone, tvb, pos, 1, FALSE);
			break;
		case 0x13:	/* location information */
			...
			proto_tree_add_item(elem_tree, hf_ctlv_loci_lac, tvb, pos+3, 2, FALSE);
			proto_tree_add_item(elem_tree, hf_ctlv_loci_cell_id, tvb, pos+5, 2, FALSE);
			break;
		case 0x19:	/* event list */
			for (i = 0; i < len; i++)
				proto_tree_add_item(elem_tree, hf_ctlv_event, tvb, pos+i, 1, FALSE);
			break;
		case 0x1b:	/* location status */
			for (i = 0; i < len; i++)
				proto_tree_add_item(elem_tree, hf_ctlv_loc_status, tvb, pos+i, 1, FALSE);
			break;
		case 0x32:	/* bearer */
			for (i = 0; i < len; i++)
				proto_tree_add_item(elem_tree, hf_ctlv_bearer, tvb, pos+i, 1, FALSE);
			break;
		case 0x35:	/* bearer description */
			proto_tree_add_item(elem_tree, hf_ctlv_bearer, tvb, pos, 1, FALSE);
			for (i = 1; i < len; i++)
				proto_tree_add_item(elem_tree, hf_ctlv_bearer_descr, tvb, pos+i, 1, FALSE);
			break;
		case 0x3c:	/* UICC/terminal interface transport level */
			if (len < 3)
				break;
			proto_tree_add_item(elem_tree, hf_ctlv_transport_ptype, tvb, pos, 1, FALSE);
			proto_tree_add_item(elem_tree, hf_ctlv_transport_port, tvb, pos+1, 2, FALSE);
			break;
		case 0x3f:	/* access technology */
			for (i = 1; i < len; i++)
				proto_tree_add_item(elem_tree, hf_ctlv_access_tech, tvb, pos+i, 1, FALSE);
			break;
		}

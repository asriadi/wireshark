...
static int
_parse_dialed_number(gchar *s, int slen, const guint8 *num)
{
	guint16 grp[5];
	int rv;

	grp[0] = ((num[0] & 0x3f) << 4) | ((num[1] & 0xf0) >> 4);
	grp[1] = ((num[1] & 0x0f) << 6) | ((num[2] & 0xfc) >> 2);
	grp[2] = ((num[2] & 0x03) << 8) | num[3];
	grp[3] = ((num[4] & 0xff) << 2) | ((num[5] & 0xc0) >> 6);
	grp[4] = ((num[5] & 0x3f) << 5) | ((num[6] & 0xf8) >> 3);
                       ...
                       int is_moc)
{
	...
	if (is_moc) {
		...
		/* Dialed number */
			/* Parse number */
		_parse_dialed_number(s, sizeof(s),
		                     tvb_get_ptr(tvb, offset + 3, 7));
